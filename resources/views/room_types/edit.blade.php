...
<div class="py-6">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('room_types.index') }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium transition">
                &larr; Back to List
            </a>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-8">
                ...
                <form action="{{ route('room_types.update', $roomType->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="type" id="type" value="{{ old('type', $roomType->type) }}" required
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" name="base_price" id="base_price" step="0.01" value="{{ old('base_price', $roomType->base_price) }}" required
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            @error('base_price') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label for="images" class="block text-sm font-medium text-gray-700 mb-1">Add New Images (Optional)</label>
                            ...
                            <div class="flex flex-wrap gap-4">
                                @foreach($roomType->images as $image)
                                    <div class="relative group">
                                        <img src="{{ asset('storage/' . $image->path) }}" alt="Room Image" class="w-24 h-24 object-cover rounded-md shadow-sm">
                                        {{-- زر حذف اختياري للصورة (يحتاج راوت منفصل إذا أردت تفعيله) --}}
                                        {{-- <button type="button" class="absolute top-0 right-0 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition text-xs">&times;</button> --}}
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 p-6 rounded-xl border border-gray-200 grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach ($services as $service)
                                <label class="inline-flex items-center cursor-pointer group">
                                    ...
                                    <span class="ml-2 text-sm text-gray-700 group-hover:text-indigo-700 transition">{{ $service->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-100">
                        <a href="{{ route('room_types.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                            Cancel
                        </a>
                        <button type="submit" class="px-6 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-md transition transform hover:-translate-y-0.5">
                            Update Changes
                        </button>
                    </div>
