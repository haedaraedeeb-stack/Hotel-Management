                ...
                @can('show room_types')
                    <div class="flex gap-3">
                        <a href="{{ route('room_types.index') }}"
                            class="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm font-medium transition">
                            Back
                        </a>
                        @can('edit room_types')
                        <a href="{{ route('room_types.edit', $roomType->id) }}"
                        ...
                        @if ($roomType->images->isNotEmpty())
                            ...
                            <div class="h-96 w-full bg-gray-100 relative">
                                <img src="{{ asset('storage/' . $roomType->images->first()->path) }}"
                                    alt="{{ $roomType->type }}" class="w-full h-full object-cover">
                            </div>
                        ...
                        @if ($roomType->services->isNotEmpty())
                            <div class="flex flex-wrap gap-2">
                                @foreach ($roomType->services as $service)
                                    <span
                                        class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-indigo-50 text-indigo-700 border border-indigo-100">
                                        {{ $service->name }}
                                    </span>
                                @endforeach
                            </div>
