...
<div class="py-6">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        {{-- Flash Message --}}
        @if(session('success'))
            <div class="mb-6 bg-green-50 border-l-4 border-green-500 p-4 rounded-md shadow-sm flex justify-between items-center">
                <div class="flex">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition duration-300 flex flex-col h-full">
                    ...
                    <div class="h-48 w-full bg-gray-100 relative group overflow-hidden">
                        @if ($type->images->isNotEmpty())
                            <img src="{{ asset('storage/' . $type->images->first()->path) }}"
                                 alt="{{ $type->type }}"
                                 class="w-full h-full object-cover transition duration-500 group-hover:scale-105">

                            {{-- Image Count Badge --}}
                            @if($type->images->count() > 1)
                                <span class="absolute top-3 right-3 bg-black/50 text-white text-xs px-2 py-1 rounded-full backdrop-blur-sm flex items-center">
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        <div class="flex justify-between items-start mb-3">
                            ...
                            <span class="bg-green-100 text-green-800 text-sm font-bold px-2.5 py-0.5 rounded border border-green-200">
                        ...
                        @if($type->services->isNotEmpty())
                            <div class="mb-4">
                                <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Includes:</span>
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    @if($type->services->count() > 3)
                                        <span class="text-xs text-gray-500 pt-1">+{{ $type->services->count() - 3 }} more</span>
                                    @endif
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="bg-gray-50 px-6 py-4 border-t border-gray-100 flex justify-between items-center">
                        <a href="{{ route('room_types.show', $type->id) }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-800 hover:underline">
                            View Details
                        </a>

                        <div class="flex items-center gap-3">
                            <a href="{{ route('room_types.edit', $type->id) }}" class="p-2 text-gray-500 hover:text-indigo-600 hover:bg-gray-200 rounded-full transition" title="Edit">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>

                            <form action="{{ route('room_types.destroy', $type->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this type?');">
                                @csrf
                                @method('DELETE')
        ...
        @if($roomTypes->isEmpty())
            <div class="text-center py-12 bg-white rounded-lg border border-dashed border-gray-300 mt-6">
                ...
                <div class="mt-6">
                    <a href="{{ route('room_types.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
